...
- **Google Generative AI & Ollama LLMs**: For natural language processing and generation, supporting multiple models:
  - **Gemini Models**: `gemini-2.0-flash`, `gemini-2.5-flash`, `gemini-2.5-pro`
  - **Ollama Models**: `llama3.2` and other locally available models
...
4. **(Optional)** For Ollama models, install Ollama locally:
...
6. The application will open in the default browser and you can start using it.
<div align="center">
...
7. **Model Setup**:
   ...
   - **For Gemini Models**: Get your free Gemini API key from [here](https://aistudio.google.com/app/apikey) and enter it in the app's sidebar
   - **For Ollama Models**: Install Ollama locally and pull the desired model (e.g., `ollama pull llama3.2`). No API key required.

   Then select your preferred model and choose your dataset option.

8. **Dataset Options**: You can either:

   - **Upload your own dataset**: Upload any CSV file from your local system
   - **Use example datasets**: Choose from the available sample datasets:
     - **Churn Dataset**: Customer churn analysis data
     - **MELB Real Estate**: Melbourne real estate market data
     - **Spotify Data**: Most streamed Spotify songs 2024

9. You can generate insights, Chat with CSV data, and predict the data using the app.

10. Make sure to `Reset Application` and refresh the page before uploading a new dataset.

11. You can even try out our deployed application [Live Demo](https://knowledgerepresentation-bitbandits.streamlit.app/)

12. Any queries or suggestions can be raised as an issue in the repository.
